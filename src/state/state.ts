...
type ActivityEntry = { lastMessage: number; lastBotActivity: number; responseCount: number }
...
type SessionEntry = {
  ...
  lastMessage: number
  queuedMessages: Array<{ text: string; user: string; username?: string; timestamp: number; mentions?: string[] }>
}
...
export class BrainState {
  ...
  lastDecision: { messageId: string; reason: TriggerReason; delay: number; timestamp: number } | null = null
  ...
  canFollowUp(channel: string, botId: string, followUpWindow: number, followUpMax: number): boolean {
  ...
  checkSessionLimit(
    ...
    onResume: (channel: string, queued: Array<{ text: string; user: string; username?: string; timestamp: number; mentions?: string[] }>) => void,
  ): boolean {
    let session = this.sessions.get(channel)
    if (!session) {
      session = { count: 0, paused: false, lastMessage: Date.now(), queuedMessages: [] }
  ...
  queueMessage(channel: string, text: string, user: string, username?: string, timestamp?: number, mentions?: string[]) {
    let session = this.sessions.get(channel)
    if (!session) {
      session = { count: 0, paused: false, lastMessage: Date.now(), queuedMessages: [] }
      this.sessions.set(channel, session)
    }
    session.queuedMessages.push({ text, user, username, timestamp: timestamp ?? Date.now(), mentions })
  }

  recordActivity(channel: string) {
    const now = Date.now()
    const existing = this.activity.get(channel) ?? { lastMessage: 0, lastBotActivity: 0, responseCount: 0 }
