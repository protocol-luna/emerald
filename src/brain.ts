...
import { evaluateSleep, SleepBehavior } from "./behavior/sleep"
import { computeDelay, shouldIgnore, shouldReact, pickReaction, shouldHesitate, pickHesitationWord, shouldForget, TriggerReason } from "./behavior/mannerisms"
import { shouldBurst, planBurst } from "./behavior/burst"
import { applyTypo, applyLetterSwap } from "./behavior/typo"
import { EmeraldConfig } from "./config"
import { MessageEvent, InEvent, OutCommand, RespondCommand, ReactionPlan, BurstPlan, ReplyStyle } from "./protocol"
...
export class Brain {
  ...
  private trySpontaneous(client: string) {
    ...
    const sleepBehavior = evaluateSleep(this.config.time_schedules, this.config.timezone)
  ...
  private emitSpontaneous(client: string) {
  }
  ...
  handleEvent(event: InEvent): Decision[] {
    switch (event.type) {
      ...
      case "bot_message":
        ...
        this.state.recordSpeaker(event.channel, event.client === "jade" ? this.botUsers.get("jade")?.userId ?? "" : "")
        return []
  ...
  private handleMessage(event: MessageEvent): Decision[] {
    ...
    const trigger = evaluateMessage(event, botUser.userId, botUser.username, this.state, {
    ...
    if (this.state.isSessionPaused(event.channel)) {
      this.state.queueMessage(event.channel, event.text, event.user, event.username, event.timestamp, event.mentions)
      return [{ type: "ignore", messageId: event.id }]
    }
    ...
    if (shouldIgnore(trigger.reason, this.config.concentration, sleepBehavior,
      this.fatigue.getIgnoreBonus(event.channel, this.config.topic_fatigue_enabled, this.config.topic_fatigue_threshold, this.config.topic_fatigue_ignore_bonus))) {
      return [{ type: "ignore", messageId: event.id }]
    }
    ...
    const willReact = shouldReact(trigger.reason, this.config.concentration, sleepBehavior)
    ...
    const hesitationWord = willHesitate ? pickHesitationWord(this.config.hesitation_words) : undefined

    const burst = shouldBurst(this.config.burst_chance)
    const burstPlan = burst ? planBurst(this.config.burst_delay_min, this.config.burst_delay_max) : undefined
